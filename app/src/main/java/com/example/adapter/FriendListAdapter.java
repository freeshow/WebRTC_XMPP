...
/**
 * Created by songxitang on 2015/11/12.
 */
public class FriendListAdapter extends BaseAdapter {
    ...
    public FriendListAdapter(Context mContext,Collection<RosterEntry> entries) {
        ...
        if (entries != null)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                {
                    UserData userData = new UserData(entry.getUser(),entry.getName());
    ...
    {
        return mUsers.get(position).nickname;
    }

    //根据登录名获取昵称
    public String findNicknameByUsername(String username)
    {
        String nickname = mUserMap.get(username);
        if (nickname == null)
        {
            nickname = username;
        }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        TextView user = (TextView) convertView.findViewById(R.id.textview_friend_list_item_nickname);
        if (getNickname(position) == null)
        {
            user.setText(getUsername(position));
        }
        else
        {
            user.setText(getNickname(position));
        }

        ImageView imageViewCall = (ImageView) convertView.findViewById(R.id.iv_friend_list_call);
        imageViewCall.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String chatJid = getUsername(position)+"@"+mServiceName;
